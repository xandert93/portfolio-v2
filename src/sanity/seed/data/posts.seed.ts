...
export const POSTS = [
  ...
  app.listen(PORT, () => {
    ...
    -d '{"title": ""}'`,
      ...
      pt.h2('Adding Centralised Error Handling'),
      pt.pWithFormatting([
        'Right now, errors are handled inline in every route. As your API grows, it is better to centralise this with Express\u2019s built-in error-handling ',
        { italic: 'middleware' },
        '. This also ',
  ...
  app.use(express.json());`,
      ...
      pt.h2('Conclusion'),
      pt.p(
        'We went from an empty folder to a fully functional REST API with routing, validation, structured error handling and basic security middleware. The biggest takeaway here is not the code itself — it is the pattern: organise routes by resource, validate input early, return consistent status codes and centralise error handling. That pattern scales from a tiny side project all the way up to production systems handling real traffic.',
      ),
    ],
